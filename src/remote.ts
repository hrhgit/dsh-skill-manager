...
const parameter = (name: string, schema: z.ZodType) => ({ name, wire: name, source: 'json' as const, codec: strict(`@ruihuahe/dsh-skill-manager/types#${name}`, schema) })
const descriptor = (method: string, parameters: readonly ReturnType<typeof parameter>[], result: z.ZodType, resultType: string) => ({ id: `@ruihuahe/dsh-skill-manager#skillManager/${method}`, service: 'skillManager', namespace: 'skillManager', method, invocation: { kind: 'direct' as const }, parameters, result: strict(`@ruihuahe/dsh-skill-manager/types#${resultType}`, result) })
const descriptors = [
...
export const TYPERT_REMOTE: TypertRemoteContribution = { package: '@ruihuahe/dsh-skill-manager', descriptors }
export const TYPERT = { package: '@ruihuahe/dsh-skill-manager', face: 'host', schemas: [], invocations: descriptors, model: { services: [], events: [], objects: [] } }
